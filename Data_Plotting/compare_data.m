...
while 1
    ...
    if check == 0
        ...
        title(['dpred1 rms = ',num2str(s1.rms_site(is)),'. dpred2 rms = ',num2str(s2.rms_site(is))])
        ...
        ax(1) = subplot(2,3,1); %Diagonal rho
        ...
        ax(2) = subplot(2,3,2); %Off diagonal rho
    ...
    site_menu = menu('','Next Site','Previous Site','Select Site from List','Select Site from Map','Plot RMS Misfit Map Comparison','Return');
    ...
    elseif site_menu == 4 %Select site from map
        
        [X, Y] = ginput_editdata(1);%get x,y points on either phase or rho axes       
    ...
    elseif site_menu == 5 %Compute rms misfit map
