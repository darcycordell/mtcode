function [out1,out2,out3] = ginput_editdata(arg1)
% This is a slightly modified version of the original ginput which avoids a
% very mysterious bug (which apparently only happens on Mac OS) whereby the
% clicked axes is not updated when using the crosshairs. The main issue
% occurs around Line 263 on the call: initialState.MouseListener = addlistener(fig,'WindowMouseMotion', @(o,e) updateCrossHair(o,initialState.CrossHair));
% It has something to do with the way the cross-hairs are updated
% automatically as the cursor moves around the figure. When the cross-hairs
% are visible, the act of clicking on the figure no longer updates the gca
% variable. Once again, this appears to only happen on a Mac, so I think it
% is something more fundamental about how Matlab communicates with the
% mouse. For now, I just comment it out.
%
%
