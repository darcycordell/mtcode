...
% Display model in M3DET window using parameters entered on screen.
    ...
    if ~isfield(H,'dat')
        ...
        prompt={'North Edge (m)','South Edge (m)','East Edge (m)','West Edge (m)'};
        dlg_title='CREATING SYNTHETIC MODEL: NO DATA LOADED';
        def={num2str(100000),num2str(-100000),num2str(100000),num2str(-100000)};
        num_lines=1;
        ...
        lomax=(east-west)/2000 +1;
        H.ny = (ceil(lomax/yspacing)+4)*yspacing; 

        lamax = (north-south)/2000 +1;
        H.nx=(ceil(lamax/xspacing)+4)*xspacing; 

        [H.d] = make_nan_data;
        H.dat = 'synthetic';
        H.mesh_rot = 0;

        H.d.x = [east east west west]';
        H.d.y = [north south north south]';
        H.cent_lat  = 0;
        H.cent_long = 0;

        [long, lat] = utm2geo(H.d.x+500000,H.d.y,0,0);

        H.d.loc(:,2) = long;
        H.d.loc(:,1) = lat;
        H.d.loc(:,3) = zeros(H.d.ns,1);
    else
