...
while ~done | ~cik
    ...
    %subplot('position',[0.1 0.2 0.8*abs(scale(2)-scale(1))/max_p abs(scale(4)-scale(3))*0.8/max_d*got(3)/got(4)]);
    model(model==10)=NaN; %Replace air values with NaN, pcolor plots NaN as white
...
%     caxis([0 1]); cmap2 = gray;
    ...
    if strcmp(zaxis,'log')
        ...
        plot(ysite,-elev-0.1-3,'kv','markersize',7,'markerfacecolor','k')
...
%     print -djpeg100 -painters 'model.jpg'
    ...
    matfile = 'hypocenters_hicks_2018.mat';
    load(matfile);
    iris_eq_hicks = hypocenters_hicks_2018;
    
    id = find(iris_eq_hicks(:,3)==0);
    iris_eq_hicks(id,:) = [];
    
    edges = [0:10:300];
    subplot(2,1,1)
    histogram(iris_eq(:,4),edges); hold on
    ylabel('Earthquake Count')
    xlabel('Depth (km)')
    title('IRIS Earthquake Database')
    axis([0 250 0 1200]); grid on
    
    subplot(2,1,2)
    histogram(iris_eq_hicks(:,4),edges)
    axis([0 250 0 20000])
    title('Hicks et al. (2014) Database')
    ylabel('Earthquake Count')
    xlabel('Depth (km)'); grid on
