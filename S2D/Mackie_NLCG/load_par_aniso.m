function [seed,kmode,modfile,datfile,tau,tau_aniso,err_floor,statshift,indsitmod]=...
              load_par_aniso(name)
%==========================================================================
% read inversion parameter file

disp('IN : load_par_aniso')
...
fid=fopen(name,'r');
line=fgetl(fid);
%added by Enci so the program can read the linux parameter file 'cmd_inv_6_11'
if strfind(line,'dongle')     
    line=fgetl(fid); 
end
seed=sscanf(line,'%s',1);
line=fgetl(fid); kmode(1)=sscanf(line,'%s',1);
line=fgetl(fid); kmode(2)=sscanf(line,'%s',1);
line=fgetl(fid); kmode(3)=sscanf(line,'%s',1);
kmode=kmode=='y';
line=fgetl(fid); modfile=sscanf(line,'%s',1);        % rho_xx model
line=fgetl(fid); modfile2=sscanf(line,'%s',1);       % rho_yy model
line=fgetl(fid); modfile3=sscanf(line,'%s',1);       % rho_zz model
line=fgetl(fid); devmod=sscanf(line,'%d',1);
...
line=fgetl(fid); tau_aniso=sscanf(line,'%f',1); 
line=fgetl(fid); tear=sscanf(line,'%d',1);
