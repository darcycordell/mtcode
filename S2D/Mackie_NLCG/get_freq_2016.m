...
% find a set of periods for all modes   %changed by Enci, fEB,2016
ifound=0; xtol=0.22;
for imode = modec:modec
    if mode_found(imode)==1
        for isite=1:nsites(imode)
            ...
            if ifound==0
                xperiod=xper;
                ifound=length(xperiod);
            else
                for i=1:length(xper)
                    if isempty(find(xperiod==xper(i)))
                        xperiod=[xperiod;xper(i)];
                        ifound=ifound+1;
                    end
                end
            end
...
%tolerance changed to 0.00022 by Enci Feb 2016
tolerance=0.00022;
ifound=0;
iper=0;
for j=1:nperiods
