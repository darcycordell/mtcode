function cik=nlcg_mdl_aniso(outroot,header,outpath)
%=============================================================================
%
...
aniso_choice=menu('Anisotropy direction','xx','yy','zz','xx/yy','xx/zz','yy/zz'); % Choose which direction to plot for anisotropic inversion, DR 15/04/2010, 17/12/2010
if aniso_choice==1
    aniso_dir='_xx';
elseif aniso_choice==2
    aniso_dir='_yy';
elseif aniso_choice==3
    aniso_dir='_zz';
elseif aniso_choice==4 % For plotting the ratio of the two horizontal anisotropic directions 
    aniso_dir='_xx';
    aniso_dir2='_yy';
elseif aniso_choice==5
    aniso_dir='_xx';
    aniso_dir2='_zz';
elseif aniso_choice==6
    aniso_dir='_yy';
    aniso_dir2='_zz';
end
...
name=[outroot,aniso_dir,'.mdl'];
if aniso_choice == 4 || aniso_choice == 5 || aniso_choice == 6 % For plotting the ratio of the two horizontal anisotropic directions DR 17/12/2010
    name2=[outroot,aniso_dir2,'.mdl']
end
if exist(name,'file')
...
if (aniso_choice == 4 || aniso_choice == 5 || aniso_choice == 6) && exist(name2,'file')  % For plotting the ratio of the two horizontal anisotropic directions DR 17/12/2010
    [ny,nz,dy,dz,model2]=load_mdl(name2,version);
    model=model-model2; % This overwrites the original model variable. As the resistivities are in log space, the ratio corresponds to the difference.
    header='Anisotropy ratio';
    rhomin=-2.; rhomax= 2;      % Plot limits for anisotropy ration (log10)
elseif (aniso_choice == 4 || aniso_choice == 5 || aniso_choice == 6)
    error(['File: ',name,' does not exist'])
end
...
while ~done | ~cik
    ...
    if strcmp(zaxis,'log')
        ...
        plot(ysite,-elev,'kv','markersize',7,'markerfacecolor','k')
...
%    hcb = colorbar('ver')
    ...
    if mancol==1
    if lmod2~=1
        if (aniso_choice == 4 || aniso_choice == 5 || aniso_choice == 6)  % Different colourbar for anisotropy ratio
            hcb = colorbar('hor','position',[(0.1+0.8*abs(scale(2)-scale(1))/max_p/2-0.125) 0.25 0.25 0.025]);%...   % MJU Manual changes here to position colour bar
                %'XTick',[-0.5,0,0.5],...
                %'FontSize',12,'fontname',font_n,'fontsize',font_s)
                %'XTickLabel',{'0.01','0.1','1','10','100'});
            set(get(hcb, 'Title'), 'String', 'Anisotropy Ratio','fontsize',12,'fontname',font_n);
        else
            hcb = colorbar('hor','position',[(0.1+0.8*abs(scale(2)-scale(1))/max_p/2-0.125) 0.25 0.25 0.025],...  % MJU Manual changes here to position colour bar
    ...
    if (aniso_choice == 1 || aniso_choice == 2 || aniso_choice == 3) % Only for anisotropic inversion, DR 17/12/2010
        %print('-depsc','-painters',['model',aniso_dir,'.eps']);
        print('-djpeg100','-painters',['model',aniso_dir,'.jpg']);
    elseif (aniso_choice == 4 || aniso_choice == 5 || aniso_choice == 6) % Only for anisotropic inversion, DR 17/12/2010
        %print('-depsc','-painters',['model',aniso_dir,aniso_dir2,'.eps']); 
        print('-djpeg100','-painters',['model',aniso_dir,aniso_dir2,'.jpg']); 
    end
    ...
    if strcmp(zaxis,'linear')
        choice=menu('Change:','resistivity limits','y limits','z limits',...
                    'vertical exageration','adjust colorbar','color/bw','nothing','quit');
    else
