function pmv20_aniso

% Plots the results of the ANISOTROPIC NLCG6 inversion
% For full version history see separate Word document

% pmv20_aniso updates all prior versions. 

% pmv20_iso plots the results of the isotropic inversion
...
% Change to 1 for isotropic and 0 for anisotropic inversion, DR 15/04/2010
%iso=menu('Isotropic?','Yes','No'); % Commented by DR 23/12/2010

cmode = {'TM';'TE';'HZ';'TMTE'}; 
type = {'rho','pha';'rho','pha';'rel','img'};
version = '6_11';

[parfile,outpath] = uigetfile({'*.par'},'pick inversion parameter file (cmd_inv_6_11 or *.par)');
parfile=[outpath,'/',parfile];
if exist(parfile,'file')
    %disp(['reading parameter file: ',parfile]);
    [seed,kmode,modfile,datfile,tau,tau_aniso,err_floor,statshift,indsitmod] = ...
        load_par_aniso(parfile);
else
...
if exist([seedp,'.log'],'file')
    [objfun] = load_log_aniso([seedp,'.log'],err_floor);
    nit=size(objfun,1)-1;
else
...
while cik==0
    ...
    switch choice
        ...
        case 6
            header=['model: ',seed,' |',hmode,' | tau=',num2str(tau),' | atau=',num2str(tau_aniso,'%03.1f'),...
                    ' | rms=',num2str(objfun(nit+1,2)),' | ',num2str(nit),' iters'];
            %header=''
            cik=nlcg_mdl_aniso(seedp,header,outpath);
        case 7
            header=['sens: ',seed,' |',hmode,' | tau=',num2str(tau),...
                         ' | rms=',num2str(objfun(nit+1,2)),' | ',num2str(nit),' iters'];
            %header=''
            nlcg_sns_aniso(seedp,header,outpath)
        case 8
